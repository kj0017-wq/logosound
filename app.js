...
const exerciseCategoryGrid = document.querySelector("#exerciseCategoryGrid");
const exerciseBrowserTitle = document.querySelector("#exerciseBrowserTitle");
const exerciseBrowserSubmenu = document.querySelector("#exerciseBrowserSubmenu");
const exerciseBrowserSubmenuTitle = document.querySelector("#exerciseBrowserSubmenuTitle");
const exerciseBrowserSubmenuMeta = document.querySelector("#exerciseBrowserSubmenuMeta");
const exerciseBrowserSubcategoryGrid = document.querySelector("#exerciseBrowserSubcategoryGrid");
const exerciseBrowserBackButton = document.querySelector("#exerciseBrowserBackButton");
const exerciseBrowserList = document.querySelector("#exerciseBrowserList");
...
const voiceAnalysisPanel = document.querySelector("#voiceAnalysisPanel");
const voiceAnalysisHome = document.querySelector("#voiceAnalysisHome");
const voiceAnalysisWorkspace = document.querySelector("#voiceAnalysisWorkspace");
const voiceAnalysisTitle = document.querySelector("#voiceAnalysisTitle");
...
const voiceAnalysisResetButton = document.querySelector("#voiceAnalysisResetButton");
const voiceAnalysisSaveButton = document.querySelector("#voiceAnalysisSaveButton");
const voiceAnalysisAudio = document.querySelector("#voiceAnalysisAudio");
const voiceAnalysisMessage = document.querySelector("#voiceAnalysisMessage");
const voiceAnalysisReferenceControls = document.querySelector("#voiceAnalysisReferenceControls");
const referenceToneSelect = document.querySelector("#referenceToneSelect");
const referenceToneCustom = document.querySelector("#referenceToneCustom");
const referenceTonePlayButton = document.querySelector("#referenceTonePlayButton");
const referenceVideoPlayButton = document.querySelector("#referenceVideoPlayButton");
const voiceAnalysisReferenceVideo = document.querySelector("#voiceAnalysisReferenceVideo");
const referenceToneResult = document.querySelector("#referenceToneResult");
const voiceAnalysisTextTraining = document.querySelector("#voiceAnalysisTextTraining");
const voiceTrainingText = document.querySelector("#voiceTrainingText");
const voiceAnalysisHistoryControls = document.querySelector("#voiceAnalysisHistoryControls");
const voiceAnalysisHistoryFilter = document.querySelector("#voiceAnalysisHistoryFilter");
const editorAnalysisControls = {
  recording: document.querySelector("#editorAnalysisRecording"),
  volume: document.querySelector("#editorAnalysisVolume"),
  pitch: document.querySelector("#editorAnalysisPitch"),
  pitchCurve: document.querySelector("#editorAnalysisPitchCurve"),
  waveform: document.querySelector("#editorAnalysisWaveform"),
  pauses: document.querySelector("#editorAnalysisPauses"),
  speechRate: document.querySelector("#editorAnalysisSpeechRate"),
  save: document.querySelector("#editorAnalysisSave"),
};
const voiceAnalysisEngine = window.LogoSoundVoiceAnalysis?.engine || null;
...
function setReferenceToneButtonPlaying(isPlaying) {
  if (!referenceTonePlayButton) return;
  const icon = referenceTonePlayButton.querySelector(".voice-analysis-action-icon");
  const label = referenceTonePlayButton.querySelector(".voice-analysis-action-label");
  referenceTonePlayButton.setAttribute("aria-pressed", String(isPlaying));
...
const VOICE_ANALYSIS_SCREENS = {
  ...
  comparison: { title: "Aufnahmevergleich", metric: "comparison", unit: "", detail: "Gespeicherte Messungen gegenüberstellen" },
  duration: { title: "Sprechdauer", metric: "duration", unit: "", detail: "Messzeit, aktive Sprache und Pausen" },
  pauses: { title: "Sprechpausen", metric: "pauses", unit: "", detail: "Pausen werden aus dem Pegel erkannt" },
  speechRate: { title: "Sprechtempo", metric: "speechRate", unit: "", detail: "WPM verfügbar mit Transkriptionsquelle" },
  dynamics: { title: "Stimmdynamik", metric: "dynamics", unit: "", detail: "Variation ohne medizinische Bewertung" },
  reference: { title: "Referenzton", metric: "reference", unit: " Hz", detail: "Referenzton und eigene Stimme vergleichen" },
  textTraining: { title: "Texttraining", metric: "monitor", unit: "", detail: "Übungstext mit Live-Analyse" },
  evaluation: { title: "Trainingsauswertung", metric: "evaluation", unit: "", detail: "Zusammenfassung der aktuellen Messung" },
  history: { title: "Verlauf", metric: "history", unit: "", detail: "Lokal gespeicherte Messungen" },
};
...
const EXERCISE_INSTRUCTIONS = {
  "Vokal A halten":
    "Atmen Sie ein. Atmen Sie aus. Atmen Sie ein. Atmen Sie vollständig aus. Atmen Sie ein. Sprechen Sie den Vokal lang und anhaltend.",
  "A E I O U":
    "Atmen Sie ein. Atmen Sie aus. Atmen Sie ein. Atmen Sie vollständig aus. Atmen Sie ein. Sprechen Sie die Vokale nacheinander lang und anhaltend.",
  "Pa Ta Ka":
...
if (patientName) patientName.value = localStorage.getItem(SELECTED_PATIENT_KEY) || patientName.value;
if (sensitivitySlider) sensitivitySlider.value = clampSensitivity(localStorage.getItem(SENSITIVITY_KEY) || sensitivitySlider.value);
if (sensitivityValue && sensitivitySlider) sensitivityValue.textContent = formatSensitivityLabel(sensitivitySlider.value);
loadRecordingKaraokeSpeedForCurrentExercise();
if (playbackVolumeSlider) playbackVolumeSlider.value = localStorage.getItem(PLAYBACK_GAIN_KEY) || playbackVolumeSlider.value;
if (playbackVolumeValue && playbackVolumeSlider) playbackVolumeValue.textContent = `${playbackVolumeSlider.value}%`;
...
if (window.location.hash.startsWith("#stimmanalyse") || window.location.hash.startsWith("#hearing")) {
  window.history.replaceState(null, "", `${window.location.pathname}${window.location.search}`);
}
try {
  setActiveView("exercises");
} catch (error) {
  console.error("LogoSound Startansicht konnte nicht geöffnet werden", error);
} finally {
...
init()
  ...
  .catch((error) => {
    ...
    if (permissionState) permissionState.textContent = "Bereit";
    if (message) message.textContent = "Ein Bereich konnte nicht geladen werden. Die App ist trotzdem bereit.";
    try {
      setActiveView("exercises");
    } finally {
      hideSplashAfterStartup();
...
recordButton.addEventListener("click", async (event) => {
  ...
  try {
    ...
    if (!streamReady && !hasActiveMediaStream()) {
      recordButton.disabled = false;
      recordButton.textContent = "\u00dcbung starten";
      message.textContent = "Kamera/Mikrofon konnten nicht aktiviert werden. Bitte erneut Übung starten tippen.";
      return;
    }

    await withTimeout(ensureCameraPreviewPlaying(), 1200).catch(() => false);
    await runAutomaticNoiseCalibration();
    const activeExercise = getActiveRecordingExercise();
    const instruction = getCurrentExerciseInstructionText();
    if (instruction) {
      showExerciseIntroScreen(activeExercise, instruction);
      recordButton.textContent = "Intro läuft";
      message.textContent = "Ansage läuft.";
      await withTimeout(
        speakExerciseInstruction(),
        Math.max(9000, Math.round(estimateSpeechDurationSeconds(instruction) * 1000) + 5000),
      ).catch(() => false);
    }
...
[editorExerciseName, editorMode, editorPatientScope, editorContent, editorVoiceInstruction, editorUseRepeats, editorRepeats, editorSpeed, editorVoiceSelect, breathingInhale, breathingHold, breathingExhale, breathingPause, breathingRepeats, breathingUseVoice, ...Object.values(editorAnalysisControls)].forEach((input) => {
...
async function finishRecording() {
  ...
  try {
    ...
    currentMetadata.analysisResults = buildStructuredVoiceAnalysis(currentMetadata);
    if (currentMetadata.analysisConfig.save && voiceAnalysisEngine?.saveSnapshot) {
      voiceAnalysisEngine.saveSnapshot({
        state: "stopped",
        elapsedSeconds: durationSeconds,
        activeSpeechSeconds: durationSeconds,
        volume: {
        ...
        waveform: roundedAmplitudes.map((value, index) => ({ time: index / Math.max(1, roundedAmplitudes.length), amplitude: value })),
        pitchHistory: roundedPitchHz.map((value, index) => ({ time: index / Math.max(1, roundedPitchHz.length), value })),
        volumeHistory: roundedVolumeLevels.map((value, index) => ({ time: index / Math.max(1, roundedVolumeLevels.length), value })),
        recordingBlob: null,
      }, `Übung: ${getExerciseLabel()}`);
    }
...
function updateSilenceNoiseCalibration(sample, now) {
  ...
  message.textContent = "Grundrauschen gemessen. Die Pause-/Sprachschwelle ist gesetzt.";
  if (settingsCalibrationStatus) {
    settingsCalibrationStatus.textContent =
      "Grundrauschen gemessen. Die automatische Pause-/Sprachschwelle ist gesetzt.";
  }
  updateCalibrationTestAudioButton();
  return false;
}
...
async function runAutomaticNoiseCalibration() {
  if (isRecording || isCalibrating) return false;
  recordButton.textContent = "Kalibrierung";
  message.textContent = "Bitte kurz still sein. LogoSound misst automatisch das Grundrauschen.";
  const started = await startCalibration({ embedded: true, returnView: "record" });
  if (!started) return false;
  const startedAtMs = performance.now();
  while (isCalibrating && calibrationNoiseState && !calibrationNoiseState.completed) {
    if (performance.now() - startedAtMs > CALIBRATION_SILENCE_MS + 1600) break;
    await wait(80);
  }
  stopCalibration({ silent: true });
  message.textContent = "Kalibrierung abgeschlossen. Die Übung startet.";
  return true;
}
function stopCalibration(options = {}) {
  ...
  if (!options.silent && settingsCalibrationStatus) {
    settingsCalibrationStatus.textContent = "Kalibrierung gespeichert: Rauschschwelle "
      + Math.round(calibrationNoiseFloor.volumeGate || VOLUME_NOISE_GATE)
      + ".";
  }
  if (!options.silent) {
    message.textContent = "Kalibrierung gespeichert: Rauschschwelle "
      + Math.round(calibrationNoiseFloor.volumeGate || VOLUME_NOISE_GATE)
      + ".";
  }
  if (!options.silent && options.restoreView && calibrationReturnView && calibrationReturnView !== document.body.dataset.activeView) {
...
function getEditorTokens() {
  const source = editorMode?.value === "vowels"
    ? getVowelExerciseParts(editorContent.value).content
    : editorContent.value;
  return source
    .split(/[\s,;]+/)
...
function getVowelExerciseParts(value = "") {
  const source = String(value || "").replace(/\s+/g, " ").trim();
  if (!source) return { content: "A", instruction: "" };
  const explicitVowels = source
    .split(/[\s,;|/\\:.-]+/)
    .map((token) => token.trim().toLocaleUpperCase("de-DE"))
    .filter((token) => /^[AEIOUÄÖÜ]$/.test(token));
  const content = explicitVowels.length ? explicitVowels.join(" ") : source;
  let instruction = source
    .replace(/[„“"']/g, "")
    .replace(/\b[AEIOUÄÖÜ]\b/gi, "")
    .replace(/\s*[:;,.|-]\s*$/g, "")
    .replace(/\s+/g, " ")
    .trim();
  if (/^[AEIOUÄÖÜ](?:\s+[AEIOUÄÖÜ])*$/.test(source.toLocaleUpperCase("de-DE"))) {
    instruction = "";
  }
  return { content, instruction };
}

function isLegacyVowelVoiceInstruction(value = "") {
  const text = String(value || "").trim();
  return text === "Sprechen Sie lang und anhaltend."
    || text === "Bitte sprechen Sie die Vokale nacheinander deutlich aus.";
}
...
function getExerciseInstruction() {
  const exercise = getActiveRecordingExercise();
  if (!exercise) return "";
  if (exercise.mode === "vowels" && (!exercise.voiceInstruction || isLegacyVowelVoiceInstruction(exercise.voiceInstruction))) {
    return getDefaultEditorVoiceInstruction("vowels");
  }
...
function getActiveRecordingExercise() {
  ...
  if (courseExercise?.exerciseId && courseExercise.exerciseId === exerciseName.value) {
    return hydrateEditorExercise({
      name: courseExercise.title || courseExercise.name || courseExercise.exerciseId || "Kursübung",
      mode: courseExercise.mode || "long_text",
      content: courseExercise.content || courseExercise.patientHint || courseExercise.script || courseExercise.title || "",
      rawContent: courseExercise.rawContent || "",
      script: courseExercise.script || courseExercise.content || courseExercise.patientHint || courseExercise.title || "",
      repeats: courseExercise.repetitions || courseExercise.repeats || 1,
      speed: courseExercise.speed || Number(recordingKaraokeSpeed?.value || 3),
      voiceInstruction: courseExercise.voiceInstruction || "",
      voiceAudioUrl: courseExercise.voiceAudioUrl || "",
      voiceAudioPath: courseExercise.voiceAudioPath || "",
...
function hydrateEditorExercise(exercise) {
  ...
  const contentLabel =
    (mode === "vowels" ? "" : exercise.contentLabel) ||
    (mode === "breathing"
  ...
  return {
    ...
    voiceInstruction: mode === "vowels"
      && (!String(exercise.voiceInstruction || "").trim() || isLegacyVowelVoiceInstruction(exercise.voiceInstruction))
      ? (vowelParts?.instruction || getDefaultEditorVoiceInstruction("vowels"))
...
function buildEditorExerciseFromForm() {
  ...
  const voiceInstruction =
    editorVoiceInstruction.value.trim() || vowelParts?.instruction || getDefaultEditorVoiceInstruction(mode);
...
function normalizeAnalysisConfig(config = {}) {
  return {
    ...
    volume: config.volume !== false,
    pitch: config.pitch !== false,
    pitchCurve: config.pitchCurve !== false,
    waveform: config.waveform !== false,
    pauses: Boolean(config.pauses),
    speechRate: Boolean(config.speechRate),
    save: config.save !== false,
  };
}

function getEditorAnalysisConfigFromForm() {
  return normalizeAnalysisConfig(Object.fromEntries(
    Object.entries(editorAnalysisControls).map(([key, input]) => [key, Boolean(input?.checked)]),
  ));
}

function applyEditorAnalysisConfig(config = {}) {
  const normalized = normalizeAnalysisConfig(config);
  Object.entries(editorAnalysisControls).forEach(([key, input]) => {
    if (input) input.checked = normalized[key];
  });
}

function buildStructuredVoiceAnalysis(metadata = {}) {
  const config = normalizeAnalysisConfig(metadata.analysisConfig);
  const duration = Number(metadata.dauerSekunden || 0);
  const result = { exerciseId: metadata.courseExerciseId || metadata.uebung || "", date: metadata.datum || new Date().toISOString(), duration };
  if (config.volume) result.volume = { average: Number(metadata.durchschnittlicherLautstaerkePegel || 0), min: 0, max: Number(metadata.maximalerLautstaerkePegel || 0) };
  if (config.pitch) result.pitch = { average: Number(metadata.durchschnittlicheStimmfrequenzHz || 0), min: Number(metadata.minimaleStimmfrequenzHz || 0), max: Number(metadata.maximaleStimmfrequenzHz || 0) };
...
function buildLocalEditorAiExercise(prompt) {
  ...
  if (kind === "vowels") {
    const items = sound ? [sound.toLocaleUpperCase("de-DE")] : ["A", "E", "I", "O", "U"];
    return {
      title: items.length === 1 ? `Vokal ${items[0]} halten` : "Vokale nacheinander",
      kind: "vowels",
...
function applyEditorExerciseToForm(exercise) {
  ...
  applyEditorAnalysisConfig(exercise.analysisConfig);
}
...
function renderAudioAnalysis(metadata = currentMetadata) {
  ...
  audioAnalysisNote.textContent = analysis.qualitaet.zuLeise
    ? "Hinweis: Die Aufnahme wirkt sehr leise. Bitte Abstand zum Mikrofon prüfen oder neu kalibrieren."
...
const EXERCISE_BROWSER_CATEGORIES = [
  { id: "text", title: "Texte & Sprache", icon: "T", hint: "Lange Texte, Dialoge, kurze Sätze" },
...
const TEXT_EXERCISE_SUBCATEGORIES = [
  { id: "short", title: "Kurze Sätze", hint: "Satzübungen", modes: ["sentences"] },
  { id: "long", title: "Langer Text", hint: "Lesetexte", modes: ["text", "long_text"] },
  { id: "dialog", title: "Dialoge", hint: "Rollen und Wechsel", modes: ["dialog"] },
  { id: "karaoke", title: "Karaoke-Text", hint: "Wort für Wort", modes: ["karaoke"] },
];

function getTextExerciseSubcategoryId(exercise) {
  const mode = normalizeEditorExerciseModeValue(exercise?.mode || "");
  const match = TEXT_EXERCISE_SUBCATEGORIES.find((subcategory) => subcategory.modes.includes(mode));
  return match?.id || "long";
}

function getExerciseBrowserCategories(items = getExerciseBrowserItems()) {
  const fixedCategoryIds = new Set(EXERCISE_BROWSER_CATEGORIES.map((category) => category.id));
  const fixedCategoryTitles = new Set(EXERCISE_BROWSER_CATEGORIES.map((category) => normalizeMediaLibraryTopic(category.title)));
  const blockedTopicTitles = new Set(["Intro", "Pause", "Pausen", "Mimik", "Gesicht und Mimik"]);
  const topicCategories = new Map();
  items.forEach((item) => {
    const topic = normalizeMediaLibraryTopic(item.topic || "");
    if (!item.isMedia || topic === "Kein Thema") return;
    const topicId = `topic:${slugify(topic)}`;
    if (fixedCategoryIds.has(topicId) || fixedCategoryTitles.has(topic) || blockedTopicTitles.has(topic)) return;
    topicCategories.set(topicId, {
      id: topicId,
      title: topic,
      icon: "⌁",
      hint: "Thema",
  ...
  return [
    ...EXERCISE_BROWSER_CATEGORIES,
    ...[...topicCategories.values()].sort((left, right) => left.title.localeCompare(right.title, "de")),
  ];
}

function getExerciseBrowserCategoryId(exercise) {
  const mode = normalizeEditorExerciseModeValue(exercise?.mode || "");
  const title = String(exercise?.name || exercise?.title || exercise?.description || "").toLocaleLowerCase("de-DE");
  if (mode === "media_pause") return "pausen";
  ...
  if (mode === "vowels" || /\b(stimme|vokal|ton|laut)\b/.test(title)) return "stimme";
  if (mode === "syllables" || /\b(pa ta ka|silbe|artikulation)\b/.test(title)) return "artikulation";
  if (["sentences", "text", "long_text", "dialog", "karaoke"].includes(mode)) return "text";
  return exercise?.isCustom ? "individuell" : "text";
}

function getExerciseBrowserItems() {
  const byKey = new Map();
  getExerciseLibraryItems().forEach((item) => {
    const key = String(item.exerciseId || item.name || item.id).trim().toLocaleLowerCase("de-DE");
    if (!key) return;
    const category = getExerciseBrowserCategoryId(item);
    const existing = byKey.get(key);
    if (!existing || (!existing.isCustom && item.isCustom)) byKey.set(key, { ...item, browserCategory: category, textSubcategory: getTextExerciseSubcategoryId(item) });
  });
  return [...byKey.values()].sort((left, right) => {
    const categories = getExerciseBrowserCategories([]);
    const leftCategoryIndex = categories.findIndex((category) => category.id === left.browserCategory);
    const rightCategoryIndex = categories.findIndex((category) => category.id === right.browserCategory);
    const categoryOrder = (leftCategoryIndex < 0 ? 999 : leftCategoryIndex) - (rightCategoryIndex < 0 ? 999 : rightCategoryIndex);
    return categoryOrder || String(left.name || "").localeCompare(String(right.name || ""), "de");
  });
}

function renderExerciseBrowser() {
  if (!exerciseCategoryGrid || !exerciseBrowserList) return;
  const items = getExerciseBrowserItems();
  const allCategories = getExerciseBrowserCategories(items);
  const counts = items.reduce((map, item) => {
    map[item.browserCategory] = (map[item.browserCategory] || 0) + 1;
    return map;
  }, {});
  const categories = allCategories.filter((category) => (counts[category.id] || 0) > 0);
  if (selectedExerciseBrowserCategory && !categories.some((category) => category.id === selectedExerciseBrowserCategory)) {
    selectedExerciseBrowserCategory = "";
    selectedExerciseBrowserSubcategory = "";
  }
  ...
  categories.forEach((category) => {
    ...
    const button = document.createElement("button");
    button.type = "button";
    button.className = `exercise-category-tile${selectedExerciseBrowserCategory === category.id ? " is-active" : ""}${category.isTopic ? " is-topic" : ""}`;
    button.innerHTML = `
      <span class="exercise-category-icon" aria-hidden="true">${escapeHtml(category.icon)}</span>
      <strong>${escapeHtml(category.title)}</strong>
      <small>${escapeHtml(category.hint)} · ${count}</small>
    `;
    button.addEventListener("click", () => {
      selectedExerciseBrowserCategory = category.id;
      selectedExerciseBrowserSubcategory = "";
      renderExerciseBrowser();
    });
    exerciseCategoryGrid.append(button);
  });

  const selectedCategory = categories.find((category) => category.id === selectedExerciseBrowserCategory);
  exerciseCategoryGrid.classList.toggle("is-hidden", Boolean(selectedCategory));
  exerciseCategoryGrid.hidden = Boolean(selectedCategory);
  if (exerciseBrowserTitle) {
    exerciseBrowserTitle.textContent = selectedCategory?.title || "Übungsbibliothek";
  }
  exerciseBrowserSubmenu?.classList.toggle("is-hidden", !selectedCategory);
  if (exerciseBrowserSubmenu) exerciseBrowserSubmenu.hidden = !selectedCategory;
  exerciseBrowserBackButton?.classList.toggle("is-hidden", !selectedCategory);
  if (!selectedCategory) return;

  let categoryItems = items.filter((item) => item.browserCategory === selectedCategory.id);
  const isTextCategory = selectedCategory.id === "text";
  if (exerciseBrowserSubcategoryGrid) {
    exerciseBrowserSubcategoryGrid.hidden = !isTextCategory;
    exerciseBrowserSubcategoryGrid.classList.toggle("is-hidden", !isTextCategory);
    exerciseBrowserSubcategoryGrid.innerHTML = "";
  }
  if (isTextCategory) {
    exerciseBrowserSubmenuTitle?.classList.remove("is-hidden");
    const countsBySubcategory = categoryItems.reduce((map, item) => {
      map[item.textSubcategory] = (map[item.textSubcategory] || 0) + 1;
      return map;
    }, {});
    const availableSubcategories = TEXT_EXERCISE_SUBCATEGORIES.filter((subcategory) => countsBySubcategory[subcategory.id]);
    if (selectedExerciseBrowserSubcategory && !availableSubcategories.some((subcategory) => subcategory.id === selectedExerciseBrowserSubcategory)) {
      selectedExerciseBrowserSubcategory = "";
    }
    const subcategoryButtons = categoryItems.length
      ? [
          { id: "", title: "Alle", hint: "Alle Textübungen", count: categoryItems.length },
          ...availableSubcategories.map((subcategory) => ({
            ...subcategory,
            count: countsBySubcategory[subcategory.id],
          })),
        ]
      : [];
    subcategoryButtons.forEach((subcategory) => {
      const button = document.createElement("button");
      button.type = "button";
      button.className = `exercise-subcategory-button${selectedExerciseBrowserSubcategory === subcategory.id ? " is-active" : ""}`;
      button.innerHTML = `
        <strong>${escapeHtml(subcategory.title)}</strong>
        <small>${escapeHtml(subcategory.hint)} · ${subcategory.count}</small>
      `;
      button.addEventListener("click", () => {
        selectedExerciseBrowserSubcategory = subcategory.id;
        renderExerciseBrowser();
      });
      exerciseBrowserSubcategoryGrid?.append(button);
    });
    const selectedSubcategory = availableSubcategories.find((subcategory) => subcategory.id === selectedExerciseBrowserSubcategory);
    if (exerciseBrowserSubmenuTitle) exerciseBrowserSubmenuTitle.textContent = selectedSubcategory?.title || "Alle Textübungen";
    if (!availableSubcategories.length && exerciseBrowserSubmenuMeta) {
      exerciseBrowserSubmenuMeta.textContent = "Noch keine Text- oder Sprachübung vorhanden.";
    }
    if (selectedExerciseBrowserSubcategory) {
      categoryItems = categoryItems.filter((item) => item.textSubcategory === selectedExerciseBrowserSubcategory);
    }
  } else if (exerciseBrowserSubmenuTitle) {
    exerciseBrowserSubmenuTitle.textContent = "";
    exerciseBrowserSubmenuTitle.classList.add("is-hidden");
  }
  if (exerciseBrowserSubmenuMeta) {
    exerciseBrowserSubmenuMeta.textContent = categoryItems.length
      ? `${categoryItems.length} Übung${categoryItems.length === 1 ? "" : "en"}`
      : "Noch keine Übung in dieser Kategorie.";
  }
  exerciseBrowserList.innerHTML = "";
  categoryItems.forEach((item) => {
    const isPause = item.mode === "media_pause";
    const card = document.createElement("article");
    card.className = `course-exercise-card exercise-browser-card${isPause ? " is-pause" : ""}`;
    card.innerHTML = `
      <div class="course-exercise-content">
        ${renderDailyPlanLibraryVisual(item, isPause)}
        <div class="course-exercise-copy">
          <strong>${escapeHtml(item.name || "Übung")}</strong>
          <small>${escapeHtml(item.description || getEditorModeLabel(item.mode))}</small>
          <div class="course-exercise-meta">
            <span>${formatCourseDuration(item.duration || 0)}</span>
            <span>${escapeHtml(getEditorModeLabel(normalizeEditorExerciseModeValue(item.mode)))}</span>
          </div>
        </div>
        <div class="course-exercise-actions">
          <button type="button" class="primary-action compact-action" data-action="open" aria-label="${escapeHtml(item.name || "Übung")} öffnen" title="Öffnen">›</button>
        </div>
      </div>
    `;
    card.addEventListener("click", (event) => {
      if (!event.target.closest("button, .course-exercise-content")) return;
      openExerciseBrowserItem(item).catch((error) => {
        message.textContent = `Übung konnte nicht geöffnet werden: ${error?.message || "Bitte erneut versuchen."}`;
      });
    });
    exerciseBrowserList.append(card);
  });
  if (!categoryItems.length) {
    exerciseBrowserList.innerHTML = '<div class="course-empty">Hier erscheinen Übungen, sobald sie im Editor oder als Medium angelegt wurden.</div>';
  }
}

async function openExerciseBrowserItem(item) {
  if (!item) return;
  if (item.isMedia) {
  ...
  const courseExercise = normalizeCourseExercise(buildCourseExercise(item), 0);
  prepareCourseExerciseInRecordView(courseExercise);
  setActiveView("record");
  if (isBreathingExercise(courseExercise)) {
    await startBreathingExercise(getActiveRecordingExercise(), { preview: true });
    return;
  }
  message.textContent = `Übung bereit: ${item.name || "Übung"}`;
  window.scrollTo({ top: 0, behavior: "smooth" });
}
...
function normalizeCourseExercise(exercise, index = 0) {
  ...
  if (!isCoursePauseExercise(normalized)) {
    ...
    const vowelParts = normalizedMode === "vowels"
      ? getVowelExerciseParts(normalized.content || normalized.script || normalized.patientHint || normalized.title || "")
      : null;
    const content = vowelParts?.content || normalized.content || normalized.script || normalized.patientHint || normalized.title || "";
    const repetitions = Math.max(1, Number(normalized.repetitions || normalized.repeats || 1));
    return {
      ...normalized,
      mode: normalizedMode,
      content,
      script: normalizedMode === "vowels"
        ? (repetitions > 1 ? buildRepeatedScript(content, repetitions) : content)
        : (normalized.script || content),
      contentLabel: normalizedMode === "vowels" ? formatExerciseContentLabel(content, normalizedMode) : normalized.contentLabel,
      voiceInstruction: normalizedMode === "vowels"
        && (!String(normalized.voiceInstruction || "").trim() || isLegacyVowelVoiceInstruction(normalized.voiceInstruction))
        ? (vowelParts?.instruction || getDefaultEditorVoiceInstruction("vowels"))
        : normalized.voiceInstruction,
      repetitions,
...
function showMediaLibraryItem(item, autoplay = false) {
  ...
  mediaLibraryPlayer?.classList.add(`is-media-${item.mediaType === "image" ? "image" : item.mediaType === "video" ? "video" : "audio"}`);
  ...
  if (player instanceof HTMLVideoElement) {
    const posterUrl = getMediaLibraryThumbnailSource(item);
    player.controls = true;
    player.loop = item.playbackMode === "loop";
    player.muted = false;
...
async function startCourseUnitMedia(exercise) {
  ...
  if (media.mediaType === "image") {
    ...
    imageElement.className = "media-library-preview course-playlist-image";
    imageElement.src = resolveAppUrl(media.downloadUrl);
    imageElement.alt = media.title || exercise?.title || "Übungsmedium";
  ...
  if (media.mediaType === "video") {
    ...
    mediaElement.style.backgroundColor = "#080f16";
...
function formatVoiceAnalysisTime(seconds) {
  const total = Math.max(0, Math.round(Number(seconds) || 0));
...
function drawVoiceAnalysisChart(snapshot, screen = activeVoiceAnalysisScreen) {
  if (!voiceAnalysisChart) return;
  resizeCanvasToDisplay(voiceAnalysisChart);
  const context = voiceAnalysisChart.getContext("2d");
  const width = voiceAnalysisChart.width;
  const height = voiceAnalysisChart.height;
  const isPitchView = screen === "pitch" || screen === "pitchCurve" || screen === "monitor";
  if (screen === "spectrogram") {
  ...
  const isPercentView = screen === "volume" || screen === "waveform" || screen === "dynamics";
  const minimum = isPercentView ? 0 : 70;
  const maximum = isPercentView ? 100 : 360;
  ...
  const toY = (value) => chartBottom - ((value - minimum) / Math.max(1, maximum - minimum)) * chartHeight;

  context.fillStyle = "#101820";
  context.fillRect(0, 0, width, height);
  const ticks = isPercentView ? [0, 25, 50, 75, 100] : [70, 140, 210, 280, 360];
  ticks.forEach((tick) => {
    const y = toY(tick);
    context.strokeStyle = "rgba(210, 235, 238, 0.14)";
    context.lineWidth = pixelRatio;
    context.beginPath();
    context.moveTo(chartLeft, y);
    context.lineTo(width, y);
    context.stroke();
    context.fillStyle = "rgba(226, 241, 243, 0.9)";
    context.font = `${11 * pixelRatio}px system-ui, -apple-system, BlinkMacSystemFont, sans-serif`;
    context.textAlign = "left";
    context.textBaseline = "middle";
    context.fillText(`${tick} ${unit}`, 6 * pixelRatio, y);
  });
  context.strokeStyle = "rgba(226, 241, 243, 0.38)";
  context.lineWidth = pixelRatio;
  context.beginPath();
...
function drawVoiceAnalysisSpectrogram(context, snapshot, width, height) {
  ...
  context.fillStyle = "#110083";
  context.fillRect(0, 0, width, height);
  const gradient = context.createLinearGradient(0, 0, 0, height);
  gradient.addColorStop(0, "rgba(12, 25, 108, 0.28)");
  gradient.addColorStop(0.65, "rgba(14, 3, 120, 0.08)");
  gradient.addColorStop(1, "rgba(10, 9, 72, 0.45)");
  context.fillStyle = gradient;
  context.fillRect(chartLeft, chartTop, chartWidth, chartHeight);

  if (count) {
    const columnWidth = Math.max(2 * pixelRatio, chartWidth / Math.max(1, count));
    for (let index = 0; index < count; index += 1) {
      const pitchItem = pitchHistory[Math.max(0, pitchHistory.length - count + index)];
      const volumeItem = volumeHistory[Math.max(0, volumeHistory.length - count + index)];
  ...
  const ticks = [70, 100, 150, 200, 250, 300, 350, 400, 450, 500];
  ticks.forEach((tick) => {
    const y = clampCanvas(toY(tick), chartTop, chartBottom);
    context.strokeStyle = tick === 70 || tick === 500 || tick % 100 === 0 ? "rgba(226, 241, 243, 0.22)" : "rgba(226, 241, 243, 0.12)";
    context.lineWidth = pixelRatio;
    context.beginPath();
    context.moveTo(chartLeft, y);
    context.lineTo(chartRight, y);
    context.stroke();
    context.fillStyle = "rgba(238, 248, 248, 0.92)";
    context.font = `${11 * pixelRatio}px system-ui, -apple-system, BlinkMacSystemFont, sans-serif`;
    context.textAlign = "left";
    context.textBaseline = "middle";
    context.fillText(`${tick} Hz`, 4 * pixelRatio, y);
  });
  context.strokeStyle = "rgba(238, 248, 248, 0.45)";
  context.lineWidth = 1.5 * pixelRatio;
  context.beginPath();
  context.moveTo(chartLeft, chartTop);
  context.lineTo(chartLeft, chartBottom);
  ...
  const pitchValues = pitchHistory.map((item) => Number(item?.value ?? item)).filter((value) => Number.isFinite(value) && value > 0);
  if (pitchValues.length) {
    const limitedValues = pitchValues.slice(-180);
    context.strokeStyle = "rgba(82, 215, 255, 0.95)";
    context.lineWidth = 2 * pixelRatio;
    context.lineJoin = "round";
    context.lineCap = "round";
    context.beginPath();
    limitedValues.forEach((value, index) => {
      const x = limitedValues.length === 1 ? chartLeft + chartWidth / 2 : chartLeft + (index / (limitedValues.length - 1)) * chartWidth;
      const y = clampCanvas(toY(clampCanvas(value, lowHz, highHz)), chartTop, chartBottom);
      if (index === 0) context.moveTo(x, y);
      else context.lineTo(x, y);
    });
    context.stroke();
  } else {
    context.fillStyle = "rgba(238, 248, 248, 0.82)";
    context.font = `${15 * pixelRatio}px system-ui, -apple-system, BlinkMacSystemFont, sans-serif`;
    context.textAlign = "center";
    context.textBaseline = "middle";
    context.fillText("Mikrofon starten", chartLeft + chartWidth / 2, chartTop + chartHeight / 2);
...
function drawVoiceAnalysisPitchChart(context, snapshot, width, height) {
  const pixelRatio = Math.max(1, window.devicePixelRatio || 1);
  const lowHz = 50;
  const highHz = 450;
  const leftAxis = 64 * pixelRatio;
  const rightPad = 14 * pixelRatio;
  const topPad = 8 * pixelRatio;
  const bottomPad = 10 * pixelRatio;
  const chartLeft = leftAxis;
  const chartRight = width - rightPad;
  const chartTop = topPad;
  const chartBottom = height - bottomPad;
  ...
  context.fillRect(0, 0, width, height);
  bands.forEach((band) => {
    const yTop = toY(Math.min(highHz, band.to));
    const yBottom = toY(Math.max(lowHz, band.from));
    context.fillStyle = band.color;
    context.fillRect(chartLeft, yTop, chartWidth, Math.max(1, yBottom - yTop));
    context.fillStyle = "rgba(12, 20, 28, 0.86)";
    context.font = `${13 * pixelRatio}px system-ui, -apple-system, BlinkMacSystemFont, sans-serif`;
    context.textAlign = "right";
    context.textBaseline = "middle";
    context.fillText(band.label, chartRight - 6 * pixelRatio, toY(band.labelHz));
  });
  context.strokeStyle = "rgba(85, 105, 112, 0.38)";
  context.lineWidth = 1.5 * pixelRatio;
  context.beginPath();
  context.moveTo(chartLeft, chartTop);
  context.lineTo(chartLeft, chartBottom);
  context.stroke();
  [50, 100, 150, 200, 250, 300, 350, 400, 450].forEach((hz) => {
    const y = toY(hz);
    context.strokeStyle = hz % 100 === 0 || hz === 50 || hz === 450 ? "rgba(82, 99, 108, 0.34)" : "rgba(82, 99, 108, 0.22)";
    context.lineWidth = pixelRatio;
    context.beginPath();
    context.moveTo(0, y);
    context.lineTo(width, y);
    ...
    context.textAlign = "left";
    context.textBaseline = "middle";
    context.fillText(`${hz} Hz`, 4 * pixelRatio, y - 2 * pixelRatio);
  });
  const values = Array.isArray(snapshot.pitchHistory)
    ? snapshot.pitchHistory.map((item) => Number(item?.value ?? item)).filter((value) => Number.isFinite(value) && value > 0)
    : [];
  if (!values.length) return;
  const limitedValues = values.slice(-90);
  context.lineWidth = 2.5 * pixelRatio;
  context.lineJoin = "round";
  context.lineCap = "round";
  context.strokeStyle = "#df1727";
  context.beginPath();
  limitedValues.forEach((value, index) => {
    const x = limitedValues.length === 1 ? chartLeft + chartWidth / 2 : chartLeft + (index / (limitedValues.length - 1)) * chartWidth;
    const y = clampCanvas(toY(clampCanvas(value, lowHz, highHz)), chartTop, chartBottom);
  ...
  limitedValues.forEach((value, index) => {
    const x = limitedValues.length === 1 ? chartLeft + chartWidth / 2 : chartLeft + (index / (limitedValues.length - 1)) * chartWidth;
    const y = clampCanvas(toY(clampCanvas(value, lowHz, highHz)), chartTop, chartBottom);
    context.fillStyle = "#084cff";
    context.beginPath();
    context.arc(x, y, 3 * pixelRatio, 0, Math.PI * 2);
    context.fill();
  });
}
...
function getSortedFiniteNumbers(values = []) {
  return values.map(Number).filter(Number.isFinite).sort((left, right) => left - right);
}

function getVoiceAnalysisPercentile(values = [], percentile = 0.5) {
...
function getVoiceAnalysisRangeShare(values = [], min = -Infinity, max = Infinity) {
  const spoken = values.map(Number).filter((value) => Number.isFinite(value) && value > 0);
  if (!spoken.length) return 0;
  const matching = spoken.filter((value) => value >= min && value < max).length;
...
function formatVoiceAnalysisDb(value, decimals = 1) {
  const numeric = Number(value) || 0;
  return `${numeric.toLocaleString("de-DE", { maximumFractionDigits: decimals, minimumFractionDigits: decimals })} dB`;
}

function renderVoiceAnalysisEvaluation(snapshot = {}) {
  const pitchValues = Array.isArray(snapshot.pitchHistory)
    ? snapshot.pitchHistory.map((item) => Number(item?.value ?? item)).filter((value) => Number.isFinite(value) && value > 0)
    : [];
  const volumeValues = Array.isArray(snapshot.volumeHistory)
    ? snapshot.volumeHistory.map((item) => Number(item?.value ?? item)).filter(Number.isFinite)
    : [];
  const pitchAverage = Number(snapshot.pitch?.average) || 0;
  const pitchMedian = getVoiceAnalysisPercentile(pitchValues, 0.5);
  const pitchHigh = getVoiceAnalysisPercentile(pitchValues, 0.95);
  const pitchLow = getVoiceAnalysisPercentile(pitchValues, 0.05);
  const volumeAverage = Number(snapshot.volume?.average) || 0;
  ...
  return `
    <div class="voice-test-result-card">
      <div class="voice-test-result-hero">
        <div class="voice-test-range-meter" aria-label="Tonhöhenbereich von 50 bis 350 Hertz">
          <span>350 Hz</span>
          <div class="voice-test-range-track"><i style="--pitch-pos:${Math.max(0, Math.min(100, ((pitchAverage - 50) / 300) * 100))}%"></i></div>
          <span>50 Hz</span>
        </div>
      ...
      <div class="voice-test-result-player">
        <button id="voiceEvaluationPlayButton" class="voice-evaluation-play-button" type="button" aria-pressed="false" ${snapshot.recordingBlob ? "" : "disabled"}>
          <span class="voice-analysis-action-icon">▶</span>
          <span class="voice-analysis-action-label">${snapshot.recordingBlob ? "Analysierte Aufnahme abspielen" : "Keine Aufnahme zum Abspielen"}</span>
        </button>
      </div>
      <div class="voice-test-result-section">
        <h3>Tonhöhe</h3>
        <dl>
          <div><dt>Durchschnitt</dt><dd>${formatVoiceAnalysisHz(pitchAverage, 1)}</dd></div>
      ...
      <div class="voice-test-result-section">
        <h3>Lautstärke</h3>
        <dl>
          <div><dt>Umgebung</dt><dd>${formatVoiceAnalysisDb(Math.max(0, volumeLow), 1)}</dd></div>
          <div><dt>Durchschnitt</dt><dd>${formatVoiceAnalysisDb(volumeAverage, 1)}</dd></div>
          <div><dt>Median</dt><dd>${formatVoiceAnalysisDb(volumeMedian, 1)}</dd></div>
          <div><dt>Hoch (95%)</dt><dd>${formatVoiceAnalysisDb(volumeHigh, 1)}</dd></div>
          <div><dt>Tief (5%)</dt><dd>${formatVoiceAnalysisDb(volumeLow, 1)}</dd></div>
          <div><dt>Sprechzeit</dt><dd>${formatVoiceAnalysisTime(snapshot.activeSpeechSeconds)}</dd></div>
          <div><dt>Pausen</dt><dd>${snapshot.pauses?.count || 0}</dd></div>
          <div><dt>Messdauer</dt><dd>${formatVoiceAnalysisTime(snapshot.elapsedSeconds)}</dd></div>
        </dl>
      </div>

    </div>`;
}
function stopVoiceAnalysisTone() {
  if (!voiceAnalysisToneOscillator) {
    setReferenceToneButtonPlaying(false);
    return;
  }
  try {
    voiceAnalysisToneOscillator.stop();
  } catch (error) {
    // The oscillator may already have ended after its previous playback.
  }
  try {
    voiceAnalysisToneOscillator.disconnect();
  } catch (error) {
    // Disconnecting an already released node is harmless.
  }
  voiceAnalysisToneOscillator = null;
  setReferenceToneButtonPlaying(false);
}


function getVoiceAnalysisReferenceVideoItem() {
  return (mediaLibraryItems || []).find((item) => {
    if (!item || item.mediaType !== "video") return false;
    return Boolean(item.downloadUrl || item.videoDownloadUrl || item.fileUrl || item.url);
  });
}

function getVoiceAnalysisReferenceVideoSource(item) {
  const source = item?.downloadUrl || item?.videoDownloadUrl || item?.fileUrl || item?.url || "";
  return source ? resolveAppUrl(String(source)) : "";
}

function setReferenceVideoButtonPlaying(isPlaying) {
  if (!referenceVideoPlayButton) return;
  const icon = referenceVideoPlayButton.querySelector(".voice-analysis-action-icon");
  const label = referenceVideoPlayButton.querySelector(".voice-analysis-action-label");
  referenceVideoPlayButton.setAttribute("aria-pressed", String(isPlaying));
...
function stopVoiceAnalysisReferenceVideo() {
  if (!voiceAnalysisReferenceVideo) return;
  try {
    voiceAnalysisReferenceVideo.pause();
  } catch (error) {
    // Pausing a released video element is harmless.
  }
  setReferenceVideoButtonPlaying(false);
}

function prepareVoiceAnalysisReferenceVideo() {
  if (!voiceAnalysisReferenceVideo) return null;
  const item = getVoiceAnalysisReferenceVideoItem();
  const source = getVoiceAnalysisReferenceVideoSource(item);
  if (!item || !source) {
    voiceAnalysisReferenceVideo.classList.add("is-hidden");
    if (referenceToneResult) referenceToneResult.textContent = "Kein Referenzvideo in der Medienbibliothek gefunden.";
    return null;
  }
  if (voiceAnalysisReferenceVideo.src !== source) voiceAnalysisReferenceVideo.src = source;
  voiceAnalysisReferenceVideo.classList.remove("is-hidden");
  return { video: voiceAnalysisReferenceVideo, item };
}
function renderVoiceAnalysisDetails(snapshot, screen) {
  if (!voiceAnalysisDetails) return;
  if (screen === "comparison") {
    const entries = voiceAnalysisEngine?.history?.() || [];
    if (entries.length < 2) {
      voiceAnalysisDetails.innerHTML = "<p class=\"field-help\">Speichere mindestens zwei Messungen, um sie zu vergleichen.</p>";
      return;
    }
    const current = entries[0];
    const previous = entries[1];
    const delta = (left, right) => `${left >= right ? "+" : ""}${Math.round((left - right) * 10) / 10}`;
    const rows = [["Messung A", current.label || "Aktuell"], ["Messung B", previous.label || "Vorher"], ["Lautstärke", delta(current.volume?.average || 0, previous.volume?.average || 0)], ["Ø Tonhöhe", `${delta(current.pitch?.average || 0, previous.pitch?.average || 0)} Hz`], ["Pitch-Variation", `${delta((current.pitch?.max || 0) - (current.pitch?.min || 0), (previous.pitch?.max || 0) - (previous.pitch?.min || 0))} Hz`], ["Dauer", `${delta(current.duration || 0, previous.duration || 0)} s`], ["Pausen", delta(current.pauses?.count || 0, previous.pauses?.count || 0)]];
    voiceAnalysisDetails.innerHTML = rows.map(([label, value]) => `<div><dt>${escapeHtml(String(label))}</dt><dd>${escapeHtml(String(value))}</dd></div>`).join("");
    return;
  }
  if (screen === "evaluation") {
    voiceAnalysisDetails.innerHTML = renderVoiceAnalysisEvaluation(snapshot);
    return;
  }
  const details = {
    ...
    speechRate: [["Wörter pro Minute", snapshot.speechRate.wordsPerMinute || "Noch kein Transkript"], ["Silben pro Minute", snapshot.speechRate.syllablesPerMinute || "Noch kein Transkript"], ["Effektive Sprechzeit", formatVoiceAnalysisTime(snapshot.activeSpeechSeconds)]],
    evaluation: null,
  }[screen] || [["Lautstärke", snapshot.volume.current], ["Tonhöhe", `${snapshot.pitch.current} Hz`], ["Dauer", formatVoiceAnalysisTime(snapshot.elapsedSeconds)]];
  voiceAnalysisDetails.innerHTML = details.map(([label, value]) => `<div><dt>${escapeHtml(String(label))}</dt><dd>${escapeHtml(String(value))}</dd></div>`).join("");
}
...
function setVoiceEvaluationPlayButtonPlaying(isPlaying) {
  const button = document.querySelector("#voiceEvaluationPlayButton");
  if (!button) return;
  const icon = button.querySelector(".voice-analysis-action-icon");
  const label = button.querySelector(".voice-analysis-action-label");
  button.setAttribute("aria-pressed", String(isPlaying));
  if (icon) icon.textContent = isPlaying ? "■" : "▶";
  if (label) label.textContent = isPlaying ? "Aufnahme stoppen" : "Analysierte Aufnahme abspielen";
}

function prepareVoiceAnalysisAudio(snapshot = voiceAnalysisEngine?.getSnapshot?.() || {}) {
  if (!voiceAnalysisAudio || !snapshot.recordingBlob) return false;
  if (voiceAnalysisAudioBlob !== snapshot.recordingBlob) {
    if (voiceAnalysisAudio.dataset.objectUrl) URL.revokeObjectURL(voiceAnalysisAudio.dataset.objectUrl);
    const objectUrl = URL.createObjectURL(snapshot.recordingBlob);
    voiceAnalysisAudio.dataset.objectUrl = objectUrl;
    voiceAnalysisAudio.src = objectUrl;
    voiceAnalysisAudioBlob = snapshot.recordingBlob;
  }
  return Boolean(voiceAnalysisAudio.src);
}
...
async function toggleVoiceEvaluationPlayback() {
  ...
  try {
    if (!voiceAnalysisAudio.paused && !voiceAnalysisAudio.ended) {
      voiceAnalysisAudio.pause();
      voiceAnalysisAudio.currentTime = 0;
      setVoiceEvaluationPlayButtonPlaying(false);
      return;
    }
    voiceAnalysisAudio.currentTime = 0;
    await voiceAnalysisAudio.play();
    setVoiceEvaluationPlayButtonPlaying(true);
    if (voiceAnalysisMessage) voiceAnalysisMessage.textContent = "Analysierte Aufnahme wird abgespielt.";
  } catch (error) {
    setVoiceEvaluationPlayButtonPlaying(false);
...
function renderVoiceAnalysis(snapshot = voiceAnalysisEngine?.getSnapshot?.() || {}, screen = activeVoiceAnalysisScreen) {
  ...
  const waveformValues = Array.isArray(snapshot.waveform)
    ...
    : [];
  const waveformCurrent = Math.round(waveformValues.at(-1) || 0);
...
function initializeVoiceAnalysisUi() {
  if (!voiceAnalysisPanel || !voiceAnalysisEngine) return;
  voiceAnalysisPanel.addEventListener("click", async (event) => {
    const target = event.target.closest("[data-analysis-screen-target]");
  ...
  referenceVideoPlayButton?.addEventListener("click", async () => {
    const prepared = prepareVoiceAnalysisReferenceVideo();
    if (!prepared) return;
    const { video, item } = prepared;
    if (!video.paused && !video.ended) {
      stopVoiceAnalysisReferenceVideo();
      if (referenceToneResult) referenceToneResult.textContent = "Referenzvideo gestoppt.";
      return;
    }
    try {
      video.muted = false;
      video.volume = 1;
      await video.play();
      setReferenceVideoButtonPlaying(true);
      if (referenceToneResult) referenceToneResult.textContent = `Referenzvideo läuft: ${item.title || item.name || "Video"}.`;
    } catch (error) {
      setReferenceVideoButtonPlaying(false);
      if (referenceToneResult) referenceToneResult.textContent = "Referenzvideo konnte nicht gestartet werden.";
    }
  });
  voiceAnalysisReferenceVideo?.addEventListener("ended", () => setReferenceVideoButtonPlaying(false));
  voiceAnalysisReferenceVideo?.addEventListener("pause", () => setReferenceVideoButtonPlaying(false));
  referenceTonePlayButton?.addEventListener("click", async () => {
    if (voiceAnalysisToneOscillator) {
      stopVoiceAnalysisTone();
      if (referenceToneResult) referenceToneResult.textContent = "Referenzton gestoppt.";
      return;
    }
    try {
      const frequency = clampCanvas(Number(referenceToneCustom?.value || referenceToneSelect?.value || 140), 60, 400);
      const AudioContextConstructor = window.AudioContext || window.webkitAudioContext;
      if (!AudioContextConstructor) throw new Error("Referenzton wird von diesem Browser nicht unterstützt.");
      stopVoiceAnalysisTone();
      const existingContext = voiceAnalysisEngine?.audioContext;
      const context = existingContext && existingContext.state !== "closed" ? existingContext : new AudioContextConstructor();
      ...
      oscillator.onended = () => {
        if (voiceAnalysisToneOscillator === oscillator) {
          voiceAnalysisToneOscillator = null;
          setReferenceToneButtonPlaying(false);
        }
      };
      oscillator.start();
      voiceAnalysisToneOscillator = oscillator;
      setReferenceToneButtonPlaying(true);
      if (context.state === "suspended") await context.resume();
      referenceToneResult.textContent = `Referenz: ${frequency} Hz. Jetzt nachsprechen.`;
    } catch (error) {
      stopVoiceAnalysisTone();
      referenceToneResult.textContent = error?.message || "Referenzton konnte nicht gestartet werden.";
    }
  });
...
exerciseBrowserBackButton?.addEventListener("click", () => {
  if (selectedExerciseBrowserSubcategory) {
    selectedExerciseBrowserSubcategory = "";
    renderExerciseBrowser();
    return;
...
navButtons.forEach((button) => {
    button.classList.toggle("is-active", button.dataset.targetView === viewName || (viewName === "record" && button.dataset.targetView === "exercises"));
  });
  ...
  if (["dailyPlans", "courses", "relaxMusic", "mediaLibrary", "myCourses", "exercises"].includes(viewName)) {
    renderCourseViews();
  }
